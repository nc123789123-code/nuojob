    ...
    <div className="min-h-screen bg-[#14101E] flex flex-col" style={{ fontFamily: "'Nunito', sans-serif" }}>
      ...
      <div className="border-b border-[#2A2438] bg-[#14101E]">
        <div className="max-w-3xl mx-auto px-5 h-10 flex items-center gap-2 text-xs text-[#8A8398]">
          ...
          <span>›</span>
          <span className="text-[#9A93AC] truncate">{post.title}</span>
        </div>
      </div>
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto px-5 py-10">
        <article className="bg-[#14101E] border border-[#2A2438] rounded-2xl p-6 sm:p-10 shadow-sm">
          <header className="mb-8">
            ...
            <h1 className="text-[#F4F0FA] text-2xl font-bold tracking-tight leading-snug mb-2">
              {post.title}
            </h1>
            <time className="text-xs text-[#9A93AC] font-medium">{post.date}</time>
          </header>

          <div className="space-y-4">
            {post.richContent ?? post.paragraphs.map((p, i) => (
              <p key={i} className="text-[#B8B0C8] text-sm leading-[1.75]">{p}</p>
            ))}
          </div>
        </article>

        <div className="mt-8 text-center">
          <Link href="/?tab=learn" className="text-sm text-[#A78BFA] hover:text-[#F4F0FA] transition-colors font-medium">
