 ...
 */
const BUYSIDE_CO_RE = /\b(capital|credit|invest|fund|asset|mgmt|advisors?|partners?|equity|wealth|hedge|alternative|alternatives|securities|portfolio|ventures?|trading|lending|markets?)\b/i;

/** Well-known buyside firms whose names don't contain obvious finance keywords */
const KNOWN_BUYSIDE_FIRMS_RE = /\b(blackstone|kkr|citadel|bridgewater|point72|two sigma|de shaw|d\.e\. shaw|renaissance|millennium|balyasny|exoduspoint|exodus point|sculptor|och.ziff|elliott|third point|pershing square|starboard|jana partners|corvex|paulson|viking global|tiger global|coatue|dragoneer|d1 capital|appaloosa|baupost|greenlight|glenview|blue ridge|lone pine|sequoia|benchmark|founders fund|general catalyst|andreessen|a16z|accel|insight partners|thoma bravo|vista equity|francisco partners|warburg pincus|advent international|eqt|permira|cinven|apax|hgcapital|hg capital|stonepeak|starwood capital|apollo|ares|carlyle|tpg|silver lake|bain capital|advent|nea|lightspeed|battery ventures|kleiner|iconiq|kerrisdale|deutsche bank|goldman sachs|morgan stanley|jpmorgan|jp morgan|barclays|ubs|credit suisse|nomura|jefferies|lazard|evercore|moelis|piper sandler|houlihan lokey|centerview|tudor|caxton|moore capital|brevan howard|winton|man group|man ahl|renaissance tech|two harbors|pimco|doubleline|loomis sayles|nuveen|pgim|metlife investments|principal asset|emory|endowment|sovereign|temasek|gic|adia|cdpq|otpp|calpers|calstrs)\b/i;
...
function isExternalJobValid(companyName: string, title: string): boolean {
  ...
  if (/\b(capital|credit|invest|fund|asset|advisors?|partners?|equity|wealth|hedge|alternative|alternatives|securities|portfolio|ventures?|trading|lending|markets?)\b/i.test(companyName)) return true;
  return false;
}
