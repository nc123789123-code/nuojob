    ...
    <div className="min-h-screen bg-[#14101E] flex items-center justify-center px-4">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          ...
          <p className="text-sm text-[#9A93AC]">Subscriber stats</p>
        </div>
        ...
        {!stats && (
          <form onSubmit={submit} className="bg-[#14101E] rounded-2xl border border-[#2A2438] p-6 space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full py-2.5 bg-[#7C3AED] text-white text-sm font-semibold rounded-xl hover:bg-[#243d6b] transition-colors disabled:opacity-50">
        ...
        {stats && (
          <div className="space-y-4">
            {/* Total */}
            <div className="bg-[#7C3AED] text-white rounded-2xl p-6 text-center">
            ...
            <p className="text-[11px] text-center text-[#9A93AC]">
              As of {new Date(stats.asOf).toLocaleString()}
            </p>

            <button onClick={() => { setStats(null); setToken(""); }}
              className="w-full text-xs text-[#9A93AC] hover:text-[#F4F0FA] transition-colors">
              Sign out
            </button>
          </div>
        )}
