...
export default function UnlockMore({ hasAccess, label, expandedLabel, expanded, onToggle, onUnlocked }: Props) {
  ...
  const btn =
    "mt-3 w-full py-2.5 text-xs font-semibold text-[#7C3AED] border border-[#7C3AED]/30 rounded-xl hover:bg-[#7C3AED]/5 transition-colors";
  ...
  return (
    <form onSubmit={submit} className="mt-3 flex flex-col sm:flex-row gap-2">
      <input
        ...
        className="flex-1 rounded-xl border border-[#c1c7cc]/60 bg-[#f8fafb] text-xs px-3 py-2.5 text-[#191c1e] placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-[#7C3AED] focus:border-transparent"
      />
      <button
        type="submit"
        disabled={submitting}
        className="px-4 py-2.5 rounded-xl bg-[#1A2B4A] text-white text-xs font-bold hover:bg-[#5b21b6] transition-colors disabled:opacity-50 whitespace-nowrap"
      >
