...
export default function UnlockMore({ hasAccess, label, expandedLabel, expanded, onToggle, onUnlocked }: Props) {
  ...
  const btn =
    "mt-3 w-full py-2.5 text-xs font-semibold text-[#396477] border border-[#396477]/30 rounded-xl hover:bg-[#396477]/5 transition-colors";
  ...
  return (
    <form onSubmit={submit} className="mt-3 flex flex-col sm:flex-row gap-2">
      ...
      <button
        type="submit"
        disabled={submitting}
        className="px-4 py-2.5 rounded-xl bg-[#1A2B4A] text-white text-xs font-bold hover:bg-[#243b63] transition-colors disabled:opacity-50 whitespace-nowrap"
      >
